...
**Where it lives.** The canonical copy goes in `.agents/skills/verify-<app>/`, the cross-tool project skills location. Add a symlink `.claude/skills/verify-<app>` → `../../.agents/skills/verify-<app>` so Claude Code registers it from the same files. If the repo already has its own project-skill convention, follow that instead and say which you used.
